...
lecture_notes = [r for r in search_data["resources"].values() if ("lecture-notes" in r["type"])]
# embed_string = [r.get('title',"") + " " + r.get('abstract',"") for r in lecture_notes]
embed_string = { r["identifier"]: r["title"] + " " + r["abstract"] for r in lecture_notes if r["abstract"] is not None and r["title"] is not None}